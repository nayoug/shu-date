...
- [x] 自动每周匹配任务（GitHub Actions 定时触发）
- [x] 取消本周匹配功能
- [x] 首页匹配倒计时显示
...
### 2026-04-28 - V0.3.0
- 新增自动每周匹配定时任务（GitHub Actions Cron）
- 新增取消本周匹配功能
- 新增首页每周匹配倒计时显示
- 新增匹配详情页（match-detail.ejs）
- 重构统一导航栏与侧边栏布局（Drawer & Bottom Sheet）
- 重构登录/注册页面
- 重构开发者工具面板
- 重构前端 CSS 和 JS 模块化架构
- 优化移动端适配
- 优化匹配结果展示
- 修复 Safari/WebKit 兼容性问题
- 修复登录页无障碍访问问题
- 修复构建版本化重复执行问题
...
### 高优先级
1. [x] 配置自动每周匹配任务 (Cron job)
2. [x] 将匹配任务与邮件通知接入自动调度
3. [ ] 优化用户画像展示
