...
- 新增圆环匹配度动画展示
- 新增匹配分析条形图（恋爱观念、生活习惯、兴趣爱好）
- 新增实时推荐功能（查看前 5 名推荐匹配）
- 新增历史匹配记录展示
- 优化页面视觉效果和动画
- 新增空状态和加载动画

### 2026-03-30 - V0.1.3
- 新增自动每周匹配定时任务（scheduler.js）
- 使用 node-cron 实现定时调度
- 每周一凌晨 5:00 (Asia/Shanghai) 自动执行匹配
- 生产环境自动启动调度器，开发环境需手动触发
- 新增 API 端点：GET /api/scheduler/status（查看调度器状态）
