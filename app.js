...
const INFO_PAGES = {
  privacy: {
    title: '隐私政策',
    pageTitle: '隐私政策',
    lead: '这是平台的用户告知说明，用于解释平台当前会收集哪些信息、这些信息会被怎样使用，以及哪些内容可能会在匹配流程中展示给别人。',
    sections: [
      {
        title: '我们会收集哪些信息',
        bullets: [
      ...
      },
      {
        ...
        bullets: [
          ...
          '在站内匹配结果页向你展示匹配对象的学校邮箱及其他部分资料；邮件通知中也可能包含对方昵称和年级。'
        ]
      },
      {
        title: '谁可以看到这些信息',
        bullets: [
          ...
          '匹配结果页当前会展示对方的学校邮箱、昵称、年级、性别、校区、兴趣爱好和 LoveType，用于匹配后联系；邮件通知中可能包含对方昵称与年级。',
      ...
      },
      {
        ...
        bullets: [
          ...
          '我们会尽量减少不必要的数据暴露，但当前不承诺达到正式商业服务的安全等级。'
        ]
      },
      {
        title: '注销与数据删除',
  ...
  guide: {
    title: '使用说明',
    pageTitle: '使用说明与风险提示',
    lead: '心有所SHU 用户告知与使用说明。下面这份说明聚焦在”如何使用”以及”使用时应注意哪些风险边界”，帮助你在现有产品能力下更稳妥地体验。',
    sections: [
      {
        title: '基本使用流程',
        bullets: [
...
- 提及一些双方的共同点/加分项，可以强调双方的共同兴趣爱好
- 给1-2个简单的聊天话题开启建议或是第一次见面建议
- 不要过于夸大或乐观，先从朋友开始，不要直接到恋人关系
- 不要使用用户A或B做完代称，使用双方的昵称
- 不要提及具体偏好程度的选择，而是采用描述性的方式，如不要说“仪式感都打分-2”，而是说“你们都不太重视仪式感，可能更适合随性自然的相处方式”

重要：请使用纯文本格式输出，不要使用markdown，不要使用任何符号（如**、##等），段落之间用换行分隔，每段1-2句话，150-250字`;
